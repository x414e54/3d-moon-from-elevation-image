...
{
    this->impl = new VulkanRendererImpl();
...
const char* VulkanRenderer::type_name = "Vulkan";

const char* VulkanRenderer::get_name()
{
    return VulkanRenderer::type_name;
}

void VulkanRenderer::setHeightMap(void* pixels, int width, int height, int bpp)
{
...
void VulkanRenderer::setParameters(const WorldParameters& params)
{
 /*   glBindBuffer(GL_UNIFORM_BUFFER, impl->params);
    glBufferData(GL_UNIFORM_BUFFER, sizeof(params), &params, GL_STATIC_DRAW);*/
}

void VulkanRenderer::render(const PlayerPosition& pos)
{
/*    glBindBuffer(GL_UNIFORM_BUFFER, impl->pos);
